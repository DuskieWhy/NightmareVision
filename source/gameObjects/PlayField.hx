...
class PlayField extends FlxTypedGroup<StrumNote> {
    ...
    public function set_scale(value:Float){
        for(strum in members){
			...
			if (strum.animation.curAnim!=null)
				anim = strum.animation.curAnim.name;
        ...
        for(note in notes){
            if(note.isSustainNote)
				note.scale.set(note.baseScaleX * value, note.baseScaleY);
            else
				note.scale.set(note.baseScaleX * value, note.baseScaleY * value);
    ...
    inline public function addNote(note:Note){
        notes.push(note);
		if (note.isSustainNote)
			note.scale.set(note.baseScaleX * scale, note.baseScaleY);
		else
			note.scale.set(note.baseScaleX * scale, note.baseScaleY * scale);
