...
class FunkinLua extends FunkinScript
{
	...
	public function new(script:String, ?name:String)
	{
		...
		try
		{
			final isString:Bool = !FileSystem.exists(script);
			
			var result = isString ? LuaL.dostring(lua, script) : LuaL.dofile(lua, script);
		...
		Lua_helper.add_callback(lua, "callScript", function(?luaFile:String, ?funcName:String, ?args:Array<Dynamic>) {
			...
			{
				cervix = Paths.getPrimaryPath(cervix);
		...
		Lua_helper.add_callback(lua, "keyJustPressed", function(name:String) {
			...
			{
				case 'left':
					key = getControl('NOTE_LEFT_P');
				case 'down':
					key = getControl('NOTE_DOWN_P');
				case 'up':
					key = getControl('NOTE_UP_P');
				case 'right':
					key = getControl('NOTE_RIGHT_P');
				case 'accept':
					key = getControl('ACCEPT');
				case 'back':
					key = getControl('BACK');
				case 'pause':
					key = getControl('PAUSE');
				case 'reset':
					key = getControl('RESET');
				case 'space':
		...
		Lua_helper.add_callback(lua, "keyPressed", function(name:String) {
			...
			{
				case 'left':
					key = getControl('NOTE_LEFT');
				case 'down':
					key = getControl('NOTE_DOWN');
				case 'up':
					key = getControl('NOTE_UP');
				case 'right':
					key = getControl('NOTE_RIGHT');
				case 'space':
		...
		Lua_helper.add_callback(lua, "keyReleased", function(name:String) {
			...
			{
				case 'left':
					key = getControl('NOTE_LEFT_R');
				case 'down':
					key = getControl('NOTE_DOWN_R');
				case 'up':
					key = getControl('NOTE_UP_R');
				case 'right':
					key = getControl('NOTE_RIGHT_R');
				case 'space':
		...
		Lua_helper.add_callback(lua, "isNoteChild", function(parentID:Int, childID:Int) {
			...
			if (parent != null && child != null) return parent.tail.contains(child);
	...
	public inline function getControl(key:String):Bool
	{
		return Reflect.getProperty(funkin.backend.PlayerSettings.player1.controls, key);
	}
		...
		if (Lua.isfunction(lua, -1) == true)
		{
			...
			else
			{
				if (result != null)
				{
					...
					Lua.pop(lua, 1);
					if (conv == null) return Function_Continue;
					return conv;
				}
			}
		}
	...
	override public function set(variable:String, data:Dynamic):Void
	{
		...
		// trace('trying to add var: ' + variable + ' with data: ' + data + ' data type: ' + Type.typeof(data));
