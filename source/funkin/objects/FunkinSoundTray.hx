...
class FunkinSoundTray extends FlxSoundTray
{
  ...
  override public function update(MS:Float):Void
  {
    ...
    var firstChild:Dynamic = __children[0];
    var firstChild:Bitmap = firstChild;
    if (firstChild.smoothing != ClientPrefs.globalAntialiasing)
    {
      for (child in __children)
      {
        var child:Dynamic = child;
        var child:Bitmap = child;
        child.smoothing = ClientPrefs.globalAntialiasing;
      }
    }
  }
