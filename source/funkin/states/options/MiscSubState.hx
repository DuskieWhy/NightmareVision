...
class MiscSubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		var option:Option = new Option('NMV Splash Screen', 
			"If unchecked, it will completely skip the splash screen upon the engine's boot up.", 
			'toggleSplashScreen', 
			BOOL, 
			true);
		addOption(option);
		...
		var option:Option = new Option('Dev Mode', 
			"If checked, traces & developer hotkeys will become available.", 
			'inDevMode', 
			BOOL, 
			true);
		addOption(option);
