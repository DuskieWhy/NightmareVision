...
import funkin.states.substates.MusicBeatSubstate;
...
class EditorPlayState extends MusicBeatSubstate
{
	...
	override function create(){
		...
		playerStrums = new PlayField(ClientPrefs.middleScroll ? (FlxG.width / 2):(FlxG.width / 2 + (FlxG.width / 4)), strumLine.y, 4, null, true, true, 0);
		opponentStrums = new PlayField(ClientPrefs.middleScroll?(FlxG.width / 2):(FlxG.width/2 - (FlxG.width/4)), strumLine.y, 4, null, false, true, 1);
		...
		opponentStrums.noteHitCallback = null;
	...
	function sayGo() {
		...
		add(go);
		FlxTween.tween(go, {y: go.y += 100, alpha: 0}, Conductor.crotchet / 1000, {
			...
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					if(daStrumTime >= startPos) {
						...
						if(!Std.isOfType(songNotes[3], String)) swagNote.noteType = funkin.states.editors.ChartingState.noteTypeList[songNotes[3]]; //Backward compatibility + compatibility with Week 7 charts
						...
						if(floorSus > 0) {
							for (susNote in 0...floorSus+1)
							{
								...
								var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrotchet * susNote) + (Conductor.stepCrotchet / FlxMath.roundDecimal(PlayState.SONG.speed, 2)), daNoteData, oldNote, true);
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					StrumPlayAnim(playerStrums, Std.int(Math.abs(daNote.noteData)) % 4, time);
