...
class AnimateSprite extends FlxAnimate {
    public function new(X:Float = 0, Y:Float = 0, ?Path:String, ?Settings:Settings)
        ...
        override function loadAtlas(Path:String) {
            ...
            if ((!Assets.exists('$Path/Animation.json') #if sys && !FileSystem.exists('$Path/Animation.json') #end) && haxe.io.Path.extension(Path) != "zip")
            {
        ...
        override function atlasSetting(Path:String):AnimAtlas
        {
            ...
            else
            {
                ...
                #if sys
                if (FileSystem.exists(jsonPath)) jsontxt = haxe.Json.parse(File.getContent(jsonPath));
