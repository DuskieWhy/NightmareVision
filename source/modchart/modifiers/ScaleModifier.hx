...
class ScaleModifier extends NoteModifier {
	...
	override function updateNote(beat:Float, note:Note, pos:Vector3, player:Int)
	{
		...
		if (getSubmodValue('noteScaleX',player) > 0 || getSubmodValue('noteScaleY',player) > 0) {
			...
			scale = getScale(note, FlxPoint.weak(note.defScale.x, note.defScale.y), note.noteData, player);
	...
	override function updateReceptor(beat:Float, receptor:StrumNote, pos:Vector3, player:Int)
	{
		...
		if (getSubmodValue('receptorScaleX',player) > 0 || getSubmodValue('receptorScaleY',player) > 0) {
			...
			scale = getScale(receptor, FlxPoint.weak(receptor.defScale.x, receptor.defScale.y), receptor.noteData, player);
	...
	{
		var subMods:Array<String> = ["squish", "stretch", "miniX", "miniY","receptorScaleX", "receptorScaleY", "noteScaleX", "noteScaleY"];
